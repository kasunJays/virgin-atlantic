...
public class FlightInfoRepositoryImpl implements FlightInfoRepository {
    ...
    @Override
    public CompletionStage<Optional<List<Flight>>> findByDate(final LocalDate date) {

        LOGGER.info("Loading flight information for " + date.getDayOfWeek());
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(requireNonNull(resource).openStream()))) {
            ...
            List<Flight> flights = reader
                    .lines()
                    .skip(1L)
                    .map( this::flight).filter(flight -> flight.days().contains(date.getDayOfWeek()))
                    .toList();

            flights.forEach(flight -> flight.days().removeIf( day -> !day.equals(date.getDayOfWeek())));
