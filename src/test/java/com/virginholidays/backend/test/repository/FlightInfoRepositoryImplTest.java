...
public class FlightInfoRepositoryImplTest {
    ...
    public void testFindByDate() throws ExecutionException, InterruptedException {
        ...
        assertTrue(maybeFlights.get().stream().allMatch(flight -> flight.days().size() == 1));
        assertTrue(maybeFlights.get().stream().allMatch(flight -> flight.days().get(0) == today.getDayOfWeek()));
